...
exports.initRoleRouter = function initRoleRouter(app) {
    ...
    app.get('/role/fetch', passport.authenticate('jwt', {session: false}), function fetchProduct(req, res) {
    ...
    app.post('/role/create', passport.authenticate('jwt', {session: false}), function createProduct(req, res) {
        var Role = require('./role.object');
    ...
    app.post('/role/update', passport.authenticate('jwt', {session: false}), function updateRole(req, res) {
        var errorHandler = function (status, message) {
            res.status(status).json({
                message: message.toString()
            });
    ...
    app.get('/role/delete', passport.authenticate('jwt', {session: false}), function deleteProducts(req, res) {
        var Role = require('./role.object');
